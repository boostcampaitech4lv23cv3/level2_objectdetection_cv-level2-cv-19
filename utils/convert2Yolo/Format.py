    ...
    def parse(csv_path, img_path):
        ...
        try:
            ...
            for line in csv_f:
                ...
                printProgressBar(progress_cnt + 1, progress_length, prefix='UDACITY Parsing:'.ljust(15),
                                 suffix='Complete',
                                 length=40)
                progress_cnt += 1
        ...
        try:
            with open("box_groups.txt", "w") as bboxGroups:
                ...
                for filename in filenames:
                    ...
                    printProgressBar(progress_cnt + 1, progress_length, prefix='KITTI Parsing:'.ljust(15),
                                     suffix='Complete',
                                     length=40)
                    progress_cnt += 1
    ...
    def save(self, data, save_path, img_path, img_type, manifest_path):
        ...
        try:
            ...
            with open(os.path.abspath(manifest_abspath), "w") as manifest_file:
                ...
                for key in data:
                    manifest_file.write(os.path.abspath(os.path.join(img_path, "".join([key.split("/")[1], img_type, "\n"]))))

                    with open(os.path.abspath(os.path.join(save_path, "".join([key.split("/")[1], ".txt"]))), "w") as output_txt_file:
