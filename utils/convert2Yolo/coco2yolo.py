...
parser.add_argument('--label', '-l', type=str, default=f"{DATA_PATH}/kfold/{parser.parse_args().datasets.lower()}/filter_{parser.parse_args().filter}/nsplit{parser.parse_args().n_split}/{parser.parse_args().type}_cv_{parser.parse_args().kfold}.json", help='directory of label folder or label file path')
...
parser.add_argument('--manifest_path', type=str, help='directory of manipast file', default=f"./filter_{parser.parse_args().filter}/nsplit{parser.parse_args().n_split}/")
