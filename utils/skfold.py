...
def application(args):
    ...
    cross_val = StratifiedGroupKFold(n_splits=args.n_split, shuffle=True)
    dst_path = args.path
    ...
    for idx, (train_idx, val_idx) in tqdm(enumerate(cross_val.split(x, y, groups)), total=args.n_split):
        ...
        train_dict["images"] = np.array(images)[train_img_ids].tolist()
        train_dict["annotations"] = annotations_df[annotations_df["image_id"].isin(train_img_ids)].to_dict("records")
        val_dict["images"] = np.array(images)[list(set(groups[val_idx]))].tolist()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path', '-p', type=str, default=f'{DATA_PATH}/kfold/coco/filter_{parser.parse_args().filter}/nsplit{parser.parse_args().n_split}')
